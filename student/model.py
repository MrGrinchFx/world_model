"""Student world model - Stabilized High-Fidelity Recurrent Architecture."""
...
class LayerNormGRUCell(nn.Module):
    def __init__(self, input_dim: int, hidden_dim: int):
        ...
        # Layer norms for each gate component keep scale variances bounded
        self.ln_input = nn.LayerNorm(3 * hidden_dim)
        self.ln_hidden = nn.LayerNorm(3 * hidden_dim)
        self.ln_candidate = nn.LayerNorm(hidden_dim)
        
        # Output norm stabilizes representation scale drift across long open-loop rollouts
...
class StudentWorldModel(nn.Module):
    def __init__(
        ...
        num_layers: int = 3,
        use_gru: bool = True,
        delta_limit: float = 8.0,  # Retains maximum precision headroom for clean targets
    ):
        super().__init__()
        ...
        for i in range(int(num_layers)):
            if i == 0:
                # Unconstrained input layer fully registers wide out-of-distribution shocks
                layers += [nn.Linear(in_dim, hidden_dim)]
            else:
                # Spectrally normalized deep hidden transitions maintain contraction mapping properties
        ...
        # Shared context mapping input dimension (hidden_dim + obs_dim + act_dim = 261)
    ...
    def initial_hidden(self, batch_size: int, device: torch.device):
        """Public interface helper used by training and evaluation harnesses."""
        if not self.use_gru or self.gru is None:
            return None
        return torch.zeros(batch_size, self.gru.hidden_size, device=device)

    def forward(self, obs_norm: torch.Tensor, act_norm: torch.Tensor, hidden=None):
        # Input Feature Squashing: Bounds explosive out-of-distribution tracking features safely
        ...
        if self.gru is not None:
            ...
            assert hidden is not None  # Narrows types cleanly for strict TorchScript compilation
        ...
        # Elastic Residual Gate: clips anomalous transitions under wide OOD inputs
